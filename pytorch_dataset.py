...
class MyMultitaskDataset(Dataset):
    """
    Custom Dataset class that loads pickle files from the given folder path and reads features, digit label,and gender label. The features are zero-padded or truncated to a fixed length of 60 time-frames and are then normalized to [-1,1]. 
