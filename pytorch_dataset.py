...
class MyMultitaskDataset(Dataset):
    """
    Custom Dataset class that loads pickled files from the given folder path and reads features, digit label,
    and gender label. The features are normalized to [-1,1] and are zero-padded or truncated to a fixed same 
    length of 60 time-frames. 
    ...
    def __getitem__(self, index) -> Dict[str, Union[np.ndarray, int]]:
        ...
        if feature.shape[1] < 60:
            ...
            feature = np.pad(feature, pad_width,
        ...
        gen_label = torch.tensor(
            [1 if sample['class'][10:][0] == 1 else 0], dtype=torch.float32)
